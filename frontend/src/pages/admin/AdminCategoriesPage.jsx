...
export function AdminCategoriesPage() {
  ...
  const toggleWholesale = useMutation({
    mutationFn: ({ categoryId, regionId, wholesaleVisible }) =>
      apiClient.post(`/admin/categories/${categoryId}/regions/${regionId}/wholesale`, { wholesale_visible: wholesaleVisible }),
    onSuccess: () => queryClient.refetchQueries({ queryKey: ["admin", "categories"] }),
  })
                        ...
                        <div className="flex flex-wrap gap-2">
                          {(category.regions ?? []).map((region) => (
                            ...
                            >
                              <span className="text-sm">{region.name}</span>
                              <Switch
                                checked={region.pivot?.wholesale_visible ?? false}
                                onCheckedChange={(checked) =>
                                  toggleWholesale.mutate({ categoryId: category.id, regionId: region.id, wholesaleVisible: checked })
                                }
                              />
                            </div>
                          ))}
      ...
      {editingCategory && (
        <LocalizedDialog
          ...
          initial={{
            ...
            show_company_directory: editingCategory.show_company_directory ?? false,
          }}
...
const LUCIDE_ICONS = [
  "Package", "Home", "Tv", "Sofa", "Car", "Shirt", "Apple", "BookOpen", "Dumbbell",
...
function LocalizedDialog({ title, initial = {}, onSubmit, onClose, isPending }) {
  ...
  const [showCompanyDirectory, setShowCompanyDirectory] = useState(initial.show_company_directory ?? false)
  useEffect(() => {
    setName(initial.name ?? "")
    setNameAr(initial.name_ar ?? "")
    setNameEn(initial.name_en ?? "")
    setIcon(initial.icon ?? "")
    setShowCompanyDirectory(initial.show_company_directory ?? false)
  }, [initial.name, initial.name_ar, initial.name_en, initial.icon, initial.show_company_directory])
  return (
    <Dialog open onOpenChange={(o) => !o && onClose()}>
      <DialogContent>
        ...
        <form
          onSubmit={(e) => {
            ...
            const payload = {
              ...
              name_en: nameEn || null,
              icon: icon || null,
              show_company_directory: showCompanyDirectory,
            }
            onSubmit(payload)
          }}
          className="space-y-4"
        >
          ...
          />
          <div className="space-y-2">
            <label className="text-sm font-medium">{t("categories.icon", "أيقونة Lucide")}</label>
            <Select value={icon || "none"} onValueChange={(v) => setIcon(v === "none" ? "" : v)}>
              <SelectTrigger>
                <SelectValue placeholder={t("categories.selectIcon", "اختر أيقونة")} />
              </SelectTrigger>
              <SelectContent>
